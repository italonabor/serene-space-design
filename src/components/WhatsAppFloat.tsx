...
export function WhatsAppFloat() {
  return (
    <div className="fixed bottom-6 right-6 z-50 flex items-center group">
      {/* Tooltip desktop */}
      <span className="mr-3 hidden md:inline-block rounded-full bg-card px-3.5 py-1.5 text-xs font-semibold text-foreground shadow-md transition-opacity duration-300 group-hover:opacity-100 opacity-90 border border-border">
        Agendar Consulta
      </span>
      ...
      <a
        ...
        className="relative flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-xl transition-all duration-300 hover:scale-110 hover:shadow-2xl hover:bg-[#20ba59] active:scale-95"
      >
        <span className="absolute -inset-1 animate-ping rounded-full bg-[#25D366] opacity-30" />
        <WhatsAppIcon className="relative h-8 w-8 fill-current" />
      </a>
    </div>
  );
}
